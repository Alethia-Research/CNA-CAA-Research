...
% \begin{figure}[htbp]
% \centering
% \includegraphics[width=0.85\textwidth]{figures/\brandfig{reward_curve_step_grpo}.png}
...
\begin{table}[h]
\centering
\caption{GSM8K OOD Evaluation Subset Benchmarks (50 Held-Out Examples)}
\label{tab:comparison_results}
\begin{tabular}{llccc}
\toprule
\textbf{Model} & \textbf{Mode} & \textbf{Pass@1 (\%)} & \textbf{Avg. Completion Len (tokens)} & \textbf{Reward @ 300} \\
\midrule
Qwen2.5-1.5B-Instruct & LF-GRPO (This Work) & \textbf{50.00\%} & \textbf{208} & \textbf{1.20} \\
Qwen2.5-1.5B-Instruct & Standard GRPO (Full) & 42.00\% & 230 & 1.02 \\
Qwen2.5-1.5B-Instruct & LF-GRPO (Run-1, Collapse) & 42.00\% & 380 & 1.02 \\
Qwen2.5-1.5B-Instruct & UnSteered Base (Zero-Shot) & 42.00\% & -- & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\textbf{LF-GRPO Resumed Final (This Work)} & \textbf{Zero-Shot (ChatML reasoning prompt)} & \textbf{50.00\% (25/50)} \\
...
LFSFT Model (Paper 1) & Zero-Shot & 62.00\% \\
...
\begin{itemize}
    ...
    \item \textbf{LF-GRPO:} Freezing $L0$--$L23$ isolates the mathematical core from gradient noise, allowing the monologue formatting benefits learned in $L24$--$L27$ to combine with the intact core, resulting in a substantial performance jump to **50.00\%** under strict zero-shot evaluation (and up to **58.00\%** in few-shot mode).
\end{itemize}
...
Specifically, we observe that when the model \textit{gets stuck} on a problem it cannot mathematically compute, the policy collapses into a repetitive state-loop, generating the same line of text or tags until it hits the \texttt{max\_new\_tokens} limit:
\begin{itemize}
    \item \textbf{Example A (Josh flipping houses - Incorrect):} The model successfully inventory-allocates but becomes deadlocked during the profit computation, repeating: \textit{``Identify quantities involved in calculation... Identify quantities remaining... Identify quantity needed to calculate profit...''}
    \item \textbf{Example B (Carla's download - Incorrect):} Gets stuck calculating the time, repeating: \textit{``Identify total time = 40 min \textbackslash n Identify total time = 40 min \textbackslash n Identify total time = 40 min...''}
    \item \textbf{Example C (Raymond \& Samantha - Incorrect):} Repeats the line \textit{``Identify the total time elapsed since Samantha became 31.''} 15 times consecutively.
\end{itemize}

Mechanistically, this ``Stuck Gear'' phenomenon represents a cognitive deadlock. Because the \textit{Central Engine} ($L0$--$L23$) is frozen and lacks the capacity to solve the OOD step, it cannot feed the next logical state to the \textit{Behavioral Periphery} ($L24$--$L27$). However, because the periphery is strictly conditioned by reinforcement learning to continue explaining and structuring, it enters a high-entropy repeating loop. This behavior provides \textbf{direct causal confirmation of spatial modularity}: the periphery acts strictly as a behavioral router rather than an independent reasoning engine.
